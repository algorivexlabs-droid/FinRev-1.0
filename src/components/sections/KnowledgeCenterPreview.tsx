...
export function KnowledgeCenterPreview() {
  return (
    <SectionWrapper id="knowledge-preview" aria-labelledby="knowledge-heading">
      <SectionHeader
        title="Knowledge Center"
        subtitle="Educational resources to empower your financial decisions and investment literacy"
        action={
          <Link
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
        {KNOWLEDGE_CATEGORIES.slice(0, 4).map((category, index) => (
          <Card
            key={category.slug}
            variant="interactive"
            padding="lg"
          ...
          >
            ...
            <Link
              href={`/knowledge-center/${category.slug}`}
              className="inline-flex items-center gap-1.5 text-sm font-bold text-teal-600 hover:text-teal-700"
            >
              Read Articles
              <ArrowRight
