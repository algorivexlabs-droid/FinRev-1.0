...
import { TrendingUp, Coins, Target, Heart, AlertTriangle } from 'lucide-react';
...
export function MarriageGoalCalculator() {
  ...
  const validate = (values: GoalInput): Partial<Record<keyof GoalInput, string>> => {
    ...
    if (!values.years || values.years < 1 || values.years > 20) {
      errs.years = 'Time horizon must be between 1 and 20 years';
    }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      <div className="mb-8 text-center">
        <span className="inline-flex items-center gap-2 rounded-full bg-pink-100 px-4 py-1.5 text-sm font-medium text-pink-700 dark:bg-pink-900/30 dark:text-pink-300">
          Goals
        </span>
        <h1 className="mt-4 font-display text-3xl font-bold text-neutral-900 dark:text-neutral-50 sm:text-4xl">
          Marriage Goal Planner
        </h1>
        <p className="mt-2 text-lg text-neutral-600 dark:text-neutral-400">
          Plan for marriage expenses with systematic investing
        </p>
      </div>
      ...
      {result && (
        <>
          ...
          <ChartPlaceholder
            title="Marriage Fund Growth Projection"
            description="Chart visualization would show corpus growth towards marriage goal"
          />
