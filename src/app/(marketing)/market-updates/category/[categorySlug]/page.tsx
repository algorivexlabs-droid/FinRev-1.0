...
import { getMarketUpdatesByCategory, getAllMarketCategories } from '@/lib/market/data';
...
export async function generateStaticParams() {
  return MARKET_UPDATE_CATEGORIES.map((cat) => ({ categorySlug: cat.slug }));
}

export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  const { categorySlug } = await params;
  const cat = MARKET_UPDATE_CATEGORIES.find((c) => c.slug === categorySlug);
  if (!cat)
    return createMetadata({
      title: 'Category Not Found',
      path: `/market-updates/category/${categorySlug}`,
    });
  return createMetadata({
    title: `${cat.name} | Market Updates`,
    description: `Latest ${cat.name.toLowerCase()} and insights. ${cat.description}`,
    path: `/market-updates/category/${categorySlug}`,
  });
}
...
export default async function MarketCategoryPage({ params }: CategoryPageProps) {
  const { categorySlug } = await params;
  const cat = MARKET_UPDATE_CATEGORIES.find((c) => c.slug === categorySlug);
  if (!cat) notFound();

  const updates = getMarketUpdatesByCategory(cat.name);
  ...
  return (
    <>
      ...
      <SectionWrapper id="updates" aria-labelledby="updates-heading">
        <SectionHeader title={`${cat.name}`} subtitle={`${updates.length} updates available`} />
      ...
      <SectionWrapper variant="alternate" id="related-categories" aria-labelledby="related-heading">
        <SectionHeader
          ...
          subtitle="Stay informed across all market topics"
        />
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {allCategories
            .filter((c) => c !== cat.name)
            .map((categoryName, index) => {
              const catData = MARKET_UPDATE_CATEGORIES.find((c) => c.name === categoryName);
              return catData ? (
